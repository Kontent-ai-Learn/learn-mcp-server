...
export const logger: Logger = {
	...
	logWithSpinnerAsync: async (operation) => {
		...
		return await runWithSpinner(operation);
	},
};

async function runWithSpinner<T>(operation: (spinner: SpinnerLog) => Promise<T>): Promise<T> {
	...
	const { success, error, data } = await tryCatchAsync(async () => {
		const result = await operation((spinnerData) => {
			state.text = spinnerData.message;
			render();
		});
		clearInterval(timer);
